...
import {
    Filter, XCircle, CheckCircle, Activity, Minus, ArrowUpDown, User, Award, Star
} from 'lucide-react';

export default function AbaControleMensal({ dados }) {
    const [filtroStatus, setFiltroStatus] = useState('todos');
    const [filtroTipo, setFiltroTipo] = useState('todos'); // Novo estado para o tipo
    const [filtroGrupo, setFiltroGrupo] = useState('todos');
    const [ordenacao, setOrdenacao] = useState({ campo: 'nome', direcao: 'asc' });

    // Extrai grupos únicos para o filtro
    const gruposDisponiveis = useMemo(() => {
        const grupos = dados.map(d => d.grupo).filter(g => g !== "Sem Grupo");
        return [...new Set(grupos)].sort();
    }, [dados]);

    // Cálculos para os botões de filtro (Contadores)
    const totaisDinamicos = useMemo(() => {
        // Primeiro filtra pelo GRUPO (que afeta todos os outros números)
        let listaPorGrupo = dados.filter(item => filtroGrupo === 'todos' || item.grupo === filtroGrupo);

        // Contagem por STATUS (respeitando filtro de grupo e tipo)
        const getCountStatus = (status) => listaPorGrupo.filter(item => {
            if (filtroTipo !== 'todos' && item.tipo !== filtroTipo) return false; // Respeita o tipo selecionado
        ...
        // Contagem por TIPO (respeitando filtro de grupo e status)
        const getCountTipo = (tipo) => listaPorGrupo.filter(item => {
            // Aplica filtro de STATUS atual para contar os tipos corretamente dentro do contexto
            ...
            if (['Pioneiro Regular', 'Pioneiro Especial', 'Missionário'].includes(item.tipo)) return tipo === 'Regular'; // Agrupa regulares
            return false;
        }).length;

        return {
            status: {
                total: getCountStatus('todos'),
                pendentes: getCountStatus('pendente'),
                entregues: getCountStatus('entregue'),
                pregaram: getCountStatus('pregaram'),
                naoPregaram: getCountStatus('nao_pregaram')
            },
            tipo: {
                todos: getCountTipo('todos'),
                pubs: getCountTipo('Publicador'),
                aux: getCountTipo('Pioneiro Auxiliar'),
                reg: getCountTipo('Regular')
            }
        };
    }, [dados, filtroStatus, filtroTipo, filtroGrupo]);

    // Processamento final da lista (Filtros + Ordenação)
    const dadosProcessados = useMemo(() => {
        let lista = dados.filter(item => {
            ...
            // Filtro TIPO (Novo)
            if (filtroTipo !== 'todos') {
                if (filtroTipo === 'Regular') {
                    if (!['Pioneiro Regular', 'Pioneiro Especial', 'Missionário'].includes(item.tipo)) return false;
                } else {
    ...
    return (
        <div className="animate-in fade-in slide-in-from-bottom-2 duration-300">
            <div className="mb-6 space-y-4">
                ...
                {/* LINHA 2: TIPO DE PUBLICADOR */}
                <div className="flex gap-1 overflow-x-auto pb-1 scrollbar-hide">
                    <BotaoFiltroSimples ativo={filtroTipo === 'todos'} onClick={() => setFiltroTipo('todos')} label="Todos os Tipos" count={totaisDinamicos.tipo.todos} />
                    <BotaoFiltroSimples ativo={filtroTipo === 'Publicador'} onClick={() => setFiltroTipo('Publicador')} label="Publicadores" count={totaisDinamicos.tipo.pubs} icon={User} />
                    <BotaoFiltroSimples ativo={filtroTipo === 'Pioneiro Auxiliar'} onClick={() => setFiltroTipo('Pioneiro Auxiliar')} label="Auxiliares" count={totaisDinamicos.tipo.aux} icon={Award} />
                    <BotaoFiltroSimples ativo={filtroTipo === 'Regular'} onClick={() => setFiltroTipo('Regular')} label="Regulares" count={totaisDinamicos.tipo.reg} icon={Star} />
                </div>

                {/* LINHA 3: GRUPOS */}
                <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-hide pt-2 border-t border-gray-100">
                    <button onClick={() => setFiltroGrupo('todos')} className={`px-4 py-1.5 rounded-full text-[10px] font-bold border transition whitespace-nowrap shadow-sm uppercase tracking-wider ${filtroGrupo === 'todos' ? 'bg-gray-800 text-white border-gray-800' : 'bg-white text-gray-500 border-gray-200 hover:bg-gray-50'}`}>TODOS OS GRUPOS</button>
                    ...
                    <table className="w-full text-sm text-left">
                        ...
                        <tbody className="divide-y divide-gray-100">
                            {dadosProcessados.length === 0 ?
                                <tr><td colSpan="6" className="p-8 text-center text-gray-500">Nenhum registro encontrado com esses filtros.</td></tr>
                                : dadosProcessados.map(pub => (
                                    <tr key={pub.id} className="hover:bg-gray-50 transition duration-150">
                                        ...
                                        <td className="px-6 py-3 text-center">
                                            {pub.entregue ? (
                                                pub.pregou ? <CheckCircle size={18} className="text-green-500 mx-auto" /> : <XCircle size={18} className="text-orange-400 mx-auto" title="Entregou relatório, mas não pregou." />
...
// Helper para Botões de Filtro Coloridos
const BotaoFiltro = ({ ativo, onClick, icon: Icon, label, count, cor = 'gray' }) => {
    ...
    return (
        <button
            onClick={onClick}
            className={`px-3 py-2 rounded-lg text-xs font-bold transition flex items-center gap-2 whitespace-nowrap border border-transparent ${styles[cor]} ${ativo ? 'shadow-md transform scale-105' : ''}`}
        >
...
// Helper para Botões Simples (Filtro de Tipo)
const BotaoFiltroSimples = ({ ativo, onClick, label, count, icon: Icon }) => (
    <button
        onClick={onClick}
        className={`px-3 py-1.5 rounded-md text-xs font-medium transition flex items-center gap-2 whitespace-nowrap border ${ativo ? 'bg-white border-blue-200 text-blue-700 shadow-sm ring-1 ring-blue-100' : 'border-transparent text-gray-500 hover:bg-gray-100'}`}
    >
        {Icon && <Icon size={12} className={ativo ? 'text-blue-500' : 'text-gray-400'} />}
        {label} <span className={`text-[10px] px-1.5 py-0.5 rounded-full ${ativo ? 'bg-blue-100 text-blue-700' : 'bg-gray-100 text-gray-500'}`}>{count}</span>
    </button>
);